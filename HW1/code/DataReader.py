...
def prepare_X(raw_X):
	...
	### YOUR CODE HERE
    flipped_raw_image = np.flip(raw_image,axis = 2)
    f_symmentry = np.sum(np.abs(raw_image-flipped_raw_image),axis=(1,2))/256
	### END YOUR CODE
    ...
    # print(bias.T.shape)
    # print(f_symmentry.shape)

    # print(f_intensity.shape)
	### END YOUR CODE

	# Stack features together in the following order.
	# [Feature 3, Feature 1, Feature 2]
	### YOUR CODE HERE
    X = np.concatenate((bias.reshape(raw_image.shape[0],1),f_symmentry.reshape(raw_image.shape[0],1),f_intensity.reshape(raw_image.shape[0],1)),axis=1)
    # print(X.shape)
    # print(X)
	### END YOUR CODE
    return X
